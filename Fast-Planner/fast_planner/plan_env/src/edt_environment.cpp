...
void EDTEnvironment::setMap(shared_ptr<LocalSDFMap> map) {
  this->localsdf_map_ = map;
  resolution_inv_ = 1 / localsdf_map_->getResolution();
}
...
double EDTEnvironment::evaluateCoarseEDT(Eigen::Vector3d& pos, double time) {
  double d1 = localsdf_map_->getDistance(pos);
  if (time < 0.0) {
    return d1;
  } else {
