...
//400*200*50
  ...
  //使用 use odometry and point cloud
...
//  by  gyl      2020.10.4
   cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(node_, "/sdf_map/cloud", 50));
   odom_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(node_, "/sdf_map/odom", 100));
...
Eigen::Vector3d SDFMap::position_change(Eigen::Vector3d pc, Eigen::Vector3d tc,Eigen::Quaterniond qc)
 {       
 ...
 void SDFMap::cloudOdomCallback(const sensor_msgs::PointCloud2ConstPtr& img,const nav_msgs::OdometryConstPtr& odom)
 {
      /* get pose */
  md_.camera_pos_(0) = odom->pose.pose.position.x;
  md_.camera_pos_(1) = odom->pose.pose.position.y;
  ...
  resetBuffer(md_.camera_pos_ - mp_.local_update_range_,  //由相机的位置md_.camera_pos_确定occ和dis两个buff的清除区域
                    md_.camera_pos_ + mp_.local_update_range_);
