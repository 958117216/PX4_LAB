...
int main(int argc, char** argv) {
  ...
  flatreferencePub_ = node.advertise<controller_msgs::FlatTarget>("reference/flatsetpoint", 1);
  cmd_vis_pub = node.advertise<visualization_msgs::Marker>("planning/position_cmd_vis", 10);
//  pos_cmd_pub = node.advertise<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/local", 10);
  traj_pub = node.advertise<visualization_msgs::Marker>("planning/travel_traj", 10);

 // local_pos_pub = node.advertise<geometry_msgs::PoseStamped> ("mavros/setpoint_position/local", 10);
